...
| **실제 Windows 화면에서 UI 검증** | ❌ **아직** ([#7](../../issues/7)) |
...
남은 빈칸은 **Windows 트레이 UI**입니다. 데몬은 매 푸시마다 windows-latest 러너에서 실제
로그를 읽어 실제 `state.json`을 쓰고, 트레이 앱은 같은 러너에서 구성·클릭 경로까지
실행됩니다. 하지만 누구도 실제 Windows 화면에서 펫과 트레이 아이콘을 **본 적이 없습니다.**
